...
%       Increase number of devisions in range to increase number of calculations.
%
% Outputs:
...
for k = 1:length(z)
    for i = 1:length(a)
        for j = 1:length(x)
            ...
            sigma_xx(j,k,i) = A(i) * ((theta_1-theta_2) + sin(theta_1-theta_2)*cos(theta_1+theta_2));
            sigma_zz(j,k,i) = A(i) * ((theta_1-theta_2) - sin(theta_1-theta_2)*cos(theta_1+theta_2));
            sigma_xz(j,k,i) = A(i) * (sin(theta_1-theta_2)*sin(theta_1+theta_2));
        end
    end
end
